...
def make_companion(
    ...
    # check if the cycle directory contains files for the default channels
    channels_in_directory = [channel for channel in ['dapi', 'fitc', 'cy3', 'cy5'] if
                             any(cycle_dir.glob(filename_pattern.format(channel=channel)))]
    # If channels argument is not provided, assume default channels
    if channels is None:
        channels = channels_in_directory
    else:
        # Remove channels that are not in the directory
        channels = [channel for channel in channels if channel in channels_in_directory]
